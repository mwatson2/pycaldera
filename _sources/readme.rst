...
Both clients provide identical functionality, with the synchronous
client simply wrapping the async one for convenience.
...
The main async client class for interacting with the spa. All operations
must be performed within an async context manager:
...
A synchronous wrapper around AsyncCalderaClient that provides the same
functionality without requiring async/await:
...
-  ``ConnectionError``: When network connection fails or API is
   unreachable
